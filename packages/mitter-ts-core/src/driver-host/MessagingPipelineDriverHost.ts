import { DeliveryEndpoint, MessagingPipelinePayload } from 'mitter-ts-models'
import MessagingPipelineDriver, { PipelineDriverSpec, PipelineSink } from './../specs/MessagingPipelineDriver'
import { KvStore, Mitter } from './..'
...
class SavedDeliveryEndpoints {
    constructor(
        public readonly deliveryEndpoints: { [driver: string]: DeliveryEndpoint } = {}
    ) {}
}
...
export class MessagingPipelineDriverHost {
    ...
    private savedDeliveryEndpoints: SavedDeliveryEndpoints
    ...
    private async initializeMessagingPipelines(): Promise<void> {
        this.pipelineDrivers.forEach(async (driver) => {
            ...
            try {
             let { initialized, pipelineDriverSpec } =
                await driver.initialize(this.mitterContext)
                ...
                if (syncedEndpoint === undefined) {
                    ...
                    operatingEndpoint = driverInitialized
                        ...
                        .then((deliveryEndpoint) =>
                            this.registerEndpoint(driverSpec , deliveryEndpoint)
                                .then((provisionedEndpoint) => provisionedEndpoint)
                        )
                        .catch((e) => {
    ...
    private async syncEndpoint(deliveryEndpoint: DeliveryEndpoint): Promise<DeliveryEndpoint|undefined> {
        return fetch(`${MitterApiGateway.centralApiUrl()}/v1/users/me/delivery-endpoints/${deliveryEndpoint.serializedEndpoint}`)
            .then((resp) => {
                return resp
            })
            .catch(() => {
    ...
    private registerEndpoint(
        ...
        return fetch(`${MitterApiGateway.centralApiUrl()}/v1/users/me/delivery-endpoints`, {
            ...
            .then((endpoint: Response & DeliveryEndpoint) => {
                this.savedDeliveryEndpoints = new SavedDeliveryEndpoints(
                    Object.assign({}, this.savedDeliveryEndpoints.deliveryEndpoints, { [driverSpec.name]: endpoint})
                )
    ...
    private syncEndpointsToStore() {
        ...
        this.kvStore.setItem(MessagingPipelineDriverHost.StoreKeys.SavedDeliveryEndpoints, this.savedDeliveryEndpoints)
